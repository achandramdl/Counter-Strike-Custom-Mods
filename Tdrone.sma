...
#define DRONE_ADMIN ADMIN_LEVEL_A

#define TASK_BOMB 6760200

new tdrones[33], camera[33], tdrone_speed[33]
new boom
new maxentities
new bool:wait_bomb[33]
new g_tdroneactive = 0
...
public plugin_init()
{
	register_plugin("AMX tdrone", "0.2.0", "KRoTaL, Fox-NL, man_s_our")
	
	register_clcmd("drop", "stop_tdrone")
	register_clcmd("tdrone", "create_tdrone")
	
	cost = register_cvar("drone_cost", "4000")
...
public client_disconnect(id)
{
	if(task_exists(54545454+id))
		remove_task(54545454+id)
	if(tdrones[id] > 0)
		set_pev(tdrones[id], pev_health, 4900.0)
}
...
public disable_sound()
{
	...
	{
		if(tdrones[players[i]] > 0)
		{
			emit_sound(tdrones[players[i]], CHAN_VOICE, "drone/ap_rotor2.wav", 0.0, ATTN_NORM, 0, PITCH_NORM)
		}
	}
}
...
public resethud_event(id)
{
	...
	if(tdrones[id] > 0)
	{
		cs_set_user_money(id, cs_get_user_money(id) + floatround((get_pcvar_float(cost) * entity_get_float(tdrones[id], EV_FL_health) - 5000) / 30), 1)
		emit_sound(tdrones[id], CHAN_VOICE, "drone/ap_rotor2.wav", 0.0, ATTN_NORM, 0, PITCH_NORM)
		remove_entity(tdrones[id])
		tdrones[id] = 0
	}
	if(camera[id] > 0)
	{
...
public create_tdrone(id,level,cid)
{
	if(tdrones[id] > 0)
	{
	...
	{
		console_print(id, "You don't have enough money ($%i needed)", get_pcvar_num(cost))
		client_print(id, print_center, "You don't have enough money ($%i needed)", get_pcvar_num(cost))
		return PLUGIN_HANDLED
	}
	...
	tdrones[id] = create_entity("info_target")
	if(tdrones[id] > 0)
	{
		entity_set_string(tdrones[id], EV_SZ_classname, "amx_tdrone")
		entity_set_model(tdrones[id], "models/tdrone.mdl")
		
		entity_set_size(tdrones[id], Float:{-12.0,-12.0,-6.0}, Float:{12.0,12.0,6.0})
		
		entity_set_origin(tdrones[id], origin)
		entity_set_vector(tdrones[id], EV_VEC_angles, angles)
		
		entity_set_int(tdrones[id], EV_INT_solid, 2)
		entity_set_int(tdrones[id], EV_INT_movetype, 5)
		entity_set_edict(tdrones[id], EV_ENT_owner, id)
		entity_set_int(tdrones[id], EV_INT_sequence, 1)
		entity_set_float(tdrones[id], EV_FL_takedamage, DAMAGE_AIM)
		entity_set_float(tdrones[id], EV_FL_health, float(health) + 5000.0)
		entity_set_float(tdrones[id], EV_FL_gravity, 1.0)
		
		tdrone_speed[id] = 300
		
		emit_sound(tdrones[id], CHAN_VOICE, "drone/ap_rotor2.wav", 0.5, ATTN_NORM, 0, PITCH_NORM)
	}
...
public destroy_tdrone(id,level,cid)
{
...
public stop_tdrone(id)
{
	if(tdrones[id] > 0)
	{
		if(tdrone_speed[id] <= 30 && tdrone_speed[id] >= -30)
		{
			tdrone_speed[id] = 0
		}
		return PLUGIN_HANDLED
...
public client_PreThink(id)
{
	...
	new ent = ent = find_ent_by_class(-1, "drone_bomb")
	while(ent > 0)
	{
		pev(ent, pev_velocity, v)
		vector_to_angle(v, a)
		set_pev(ent, pev_angles, a)
		ent = find_ent_by_class(ent, "drone_bomb")
	}
	if(is_user_alive(id) && tdrones[id] > 0 && camera[id] > 0)
	{
		new Float:forigin[3], Float:camera_origin[3]
		new button, oldbutton
		new tdroneId = tdrones[id]
		new Float:frame
		new Float:angles[3], Float:angle[3], Float:velocity[3]
		if(entity_get_float(tdroneId, EV_FL_health) < 5000)
		{
			new Float:explosion[3]
			entity_get_vector(tdroneId, EV_VEC_origin, explosion)
		...
		frame = entity_get_float(tdroneId, EV_FL_frame)
		if(frame < 0.0 || frame > 254.0)
		{
			entity_set_float(tdroneId, EV_FL_frame, 0.0)
		}
		else
		{
			entity_set_float(tdroneId, EV_FL_frame, frame + 1.0)
		}
		
		entity_get_vector(tdroneId, EV_VEC_origin, forigin)
		...
		if(tdrone_speed[id] > 1000)
		{
			tdrone_speed[id] = 1000
		}
		if(tdrone_speed[id] < 300)
		{
			tdrone_speed[id] = 300
		}
		...
		entity_get_vector(tdroneId, EV_VEC_origin, forigin)
		entity_get_vector(id, EV_VEC_v_angle, angles)
		pev(tdroneId, pev_angles, angle)
		angle[0] = -angle[0]
		angle_vector(angle, 1, velocity)
		angles[0] = - angles[0] - angle[0]
		angles[1] += angle[1]
		angles[2] += angle[2]
		velocity[0] *= tdrone_speed[id]
		velocity[1] *= tdrone_speed[id]
		velocity[2] *= tdrone_speed[id]
		entity_set_vector(tdroneId, EV_VEC_velocity, velocity)
		entity_set_vector(camera[id], EV_VEC_angles, angles)
		...
		if(PointContents(forigin) == CONTENTS_SOLID)
		{
			...
			entity_set_origin(tdroneId, forigin)
		}
		angle[0] += 90
		new Float:cam[3]
		...
		if(button & IN_ATTACK && !wait_bomb[id] && get_num_ents() < (maxentities - 50))
		{
			new ent = create_entity("info_target")
			if(ent > 0)
			{
				entity_set_string(ent, EV_SZ_classname, "drone_bomb")
		...
		pev(tdroneId, pev_angles, angles)
		...
		set_pev(tdroneId, pev_angles, angles)
		set_hudmessage(255, 255, 255, -2.0, 0.76, 0, 1.0, 0.01, 0.1, 0.2, 4)
		show_hudmessage(id, " [drone] Speed: %i, Health: %i", tdrone_speed[id], floatround(entity_get_float(tdrones[id], EV_FL_health) - 5000))
	}
...
public pfn_touch(entity1, entity2)
{
	...
	if(is_valid_ent(entity1))
	{
		...
		if(equal(classname, "drone_bomb"))
		{
			new Float:explosion[3]
			entity_get_vector(entity1, EV_VEC_origin, explosion)
			HL_RadiusDamage(explosion, attacker,0,200.0,300.0)
			message_begin(MSG_BROADCAST, SVC_TEMPENTITY)
			write_byte(3)
			write_coord(floatround(explosion[0]))
			write_coord(floatround(explosion[1]))
			...
			if(equal(classname2, "func_vehicle") || equal(classname2, "func_tracktrain"))
				damagecar(entity2, 50)
		}
		if(equal(classname, "amx_tdrone"))
		{
			set_pev(entity1, pev_health, 4900.0)
		}
	}
...
{
	boom = precache_model("sprites/steam1.spr")
	
	precache_model("models/tdrone.mdl")
...
public HL_RadiusDamage( Float:vecSrc[3], pevAttacker, pevInflictor, Float:flDamage, Float:flRadius)
{
	new pEntity;
	new tr;
	...
	while ((pEntity = engfunc( EngFunc_FindEntityInSphere, pEntity, vecSrc, flRadius )) != 0)
	{
		if ( pev( pEntity, pev_takedamage ) != DAMAGE_NO )
		{
			...
			take_damage(pevAttacker, pEntity, flAdjustedDamage );
		}
	}
}
}
